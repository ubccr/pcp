...
class reformat_counters:
    ...
    def __init__(self, job, name, event_map):
        ...
        stats = None
        if name in job.savehost.stats:
            stats = job.savehost.stats[name]
        ...
        if name in job.savehost.stats:
            for dev, array in job.savehost.stats[name].iteritems():
                devidx = 0
                for j in self.ctl_registers:
                    settings = array[:,j]
