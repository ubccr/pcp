...
class TaccStatsReader(object):
    ...
    def __init__(self):
        ...
        # Need to store a begin location to have a valid perfctr state
        # A file with no begins cannot process the perfctrs, since they will never be set to a valid state
        self.first_begin = -1
    ...
    def processmark(self, line):
        ...
        if action == "begin" and jobid not in self.activejobs:
            ...
            if self.first_begin < 0:
                self.first_begin = self.timestamp
    ...
    def process_stats(self):
        ...
        # Need to provide the counter routines a valid initial register state
        # This only gets set at begin
        self.good_counter_idx = -1
        if self.first_begin >= 0:
            # CPU metrics should exist at all time points
            self.good_counter_idx = self.timedict["%s%s" % ("cpu", "0")][self.first_begin]
        else:
            print "No begin records found, no perfctr state!!!!!"
            pass
...
class TaccStats2PCPEngine(object):
    ...
    def get_metric_value(self, type, dev, tmet, tindex, time_idx ):
        ...
        # check for bogus perf events
        if "intel_" in type or "amd64_" in type:
            #pprint.pprint(metric_vals)
            #if time_idx == 186 and type == "intel_snb":
            if 9988998899 in metric_vals:
                #pprint.pprint(metric_vals)
                #print "Got bad value for: %s, %s, %s, %s" %(type, dev, tmet, time_idx)
                cur_value = -9988998899
                    ...
                    if sample_time in self.tsdata.timedict["%s%s" % (type, dev)]:
                        ...
                        for e in schema.itervalues():
                            ...
                            if tmet not in  pcpmapping.pcpmappingdict[type]:
                                #print "Can't deal with metric: %s (%s)" % (tmet, type)
                                continue
                            pcpname, cur_value = self.get_metric_value( type, dev, tmet, tindex, time_idx )

                            yield ( type, dev, tmet, pcpname, cur_value )
                    else:
                        #print "No %s:%s at time: %s" % (type, dev, sample_time)
                        pass
            else:
                #print "Can't deal with type: %s" % type
                pass
...
class TaccStats2PCP(object):
    ...
    def setup_mappings(self):
                    ...
                    for metric in pcpmapping.pcpmappingdict[type]:
                        ...
                        if p_indom != c_api.PM_IN_NULL:
                            ...
                            if instance_id not in self.added_instances:
                                ...
                                #print "Adding instance: %s=%s" % (instance_id, iname)
        ...
        # State for the perf ctr metrics
        self.pcpout.pmiAddMetric( "perfevent.active", pmi.pmiLogImport.pmiID(127, 0, 1), 3, c_api.PM_IN_NULL, 4, pmapi.pmContext.pmParseUnitsStr("")[0] )
    ...
    def write_metrics(self):
        ...
        # Errors in the tacc_stats files can cause jumps back in time.
        prev_time = -1

        m = len(self.tsreader.times)
        # Loop through all the times
        for i in xrange(0, m):
            # Do we have good perfctr state for this timestep
            perfctr_status = 1
            #print "Loop %d at time : %f" % (i, self.tsreader.times[i])
            now_time = self.tsreader.times[i]
            if now_time <= prev_time:
                # Just skip this whole timestep
                # Can't really deal with this in any other way
                continue
            prev_time = now_time
            for type, dev, tmet, pcpname, cur_value in self.ts2pcp.itermetrics( i ):

                # Hacky for bad perfctr metrics
                if ("intel_" in type or "amd64_" in type) and cur_value == -9988998899:
                    perfctr_status = 0
                    continue
            ...
            # Perfctr status
            self.pcpout.pmiPutValue( "perfevent.active", "", "%d" % perfctr_status);
